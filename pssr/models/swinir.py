...
Source code of SwinIR has been modified for use in PSSR.
"""
...
class SwinIR(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        if len(depths) != len(num_heads): raise ValueError(f"Lengths of depths and num_heads must be equal. Given lengths are {len(depths)} and {len(num_heads)}.")

        channels = _force_list(channels)
        channels = channels*2 if len(channels) == 1 else channels
        num_in_ch, num_out_ch = channels[0], channels[1]
        num_feat = 64
        self.img_range = 1
        if channels == 3:
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = UpsampleOneStep(scale, embed_dim, num_out_ch,
    ...
    def extra_repr(self):
        return f"SwinIR with {self.upscale}x upscaling\n{self.num_layers} Swin Transformer blocks with embedding size {self.embed_dim}"
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if not 0 <= self.shift_size < self.window_size: raise ValueError(f"shift_size must between 0 and window_size. Given values are {shift_size} and {window_size}.")
...
class Upsample(nn.Sequential):
    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            ...
            raise ValueError(f"scale {scale} is not supported. Supported scales are 2^n and 3.")
        super().__init__(*m)
